-- 043_fix_user_camps_to_user_activities.sql
-- HOTFIX: migration 040 re-declared get_shared_planner_by_token using a body
-- copied from 029, which still referenced the old `user_camps` table name.
-- That table was renamed to `user_activities` in migration 031, so every
-- /schedule/[token] request was failing with `relation "user_camps" does not
-- exist` and rendering a 404. This migration re-declares the function with
-- the corrected table name. The only line that changes vs. 040 is
-- `from user_camps uc` → `from user_activities uc` inside the
-- `color_by_activity_id` clause. All other behavior is identical.
...
comment on function get_shared_planner_by_token(text) is
  ...
  '043 hotfix: corrects user_camps -> user_activities (renamed in 031).';
