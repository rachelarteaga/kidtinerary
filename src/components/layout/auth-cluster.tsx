    ...
    <div className="relative inline-block" ref={ref}>
      ...
      {open && (
        <div className="absolute z-20 top-full right-0 mt-2 bg-surface border border-ink rounded-xl shadow-[3px_3px_0_0_rgba(0,0,0,0.15)] p-1 min-w-[240px]">
          ...
          <Link href="/account/profile" className="block px-3 py-2 rounded-md font-sans text-[13px] font-medium text-ink hover:bg-base">Edit profile</Link>
          ...
          <Link
            ...
            onClick={(e) => {
              // Planner page relies on RSC-cached planner data keyed by id.
              // Soft-nav from /planner to /account/planners can show stale
              // names/share state that changed in the planner UI. Force a
              // full reload so the list re-fetches server-side.
              if (onPlannerPage) {
                e.preventDefault();
                setOpen(false);
            ...
            className="block px-3 py-2 rounded-md font-sans text-[13px] font-medium text-ink hover:bg-base"
          >
            My planners
          </Link>
